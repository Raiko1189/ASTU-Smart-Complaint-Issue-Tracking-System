        ...
        <>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {user?.role === 'admin' && (
                        <div className="card p-6 border-primary-200 dark:border-primary-900/50 bg-primary-50/50 dark:bg-primary-900/10">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="font-semibold text-primary-900 dark:text-primary-300 flex items-center gap-2">
                                    <span className="text-xl">🤖</span> AI Insight Summary (Admin Only)
                                </h3>
                                <button onClick={() => fetchSummary()} disabled={summaryLoading} className="btn-secondary text-xs px-3 py-1.5 border-primary-200 dark:border-primary-800">
                                    {summaryLoading ? 'Generating...' : 'Regenerate'}
                                </button>
                            </div>
                            {summaryData ? (
                                <p className="text-sm text-primary-800 dark:text-primary-200 bg-white/50 dark:bg-black/20 p-4 rounded-xl shadow-inner border border-primary-100 dark:border-primary-800/30 font-medium leading-relaxed">{summaryData}</p>
                            ) : (
                                <p className="text-sm text-primary-600/60 dark:text-primary-400/60 italic p-4 text-center">Click regenerate to analyze this complaint history.</p>
                            )}
                        </div>
                    )}
                    ...
                    {canUpdate && (
                        <div className="card p-6">
                            <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Official Update</h3>
                            <div className="space-y-4 bg-slate-50 dark:bg-white/5 p-5 rounded-2xl border border-slate-100 dark:border-white/5">
                                <div>
                                    ...
                                    <select value={status} onChange={(e) => setStatus(e.target.value)} className="input-field bg-white dark:bg-dark-card font-semibold">
                                        ...
                                        {user?.role === 'admin' && <option value="Resolved">Force Resolve (Admin Override)</option>}
                                    </select>
                                </div>
